...
Retrieval Phase — Custom CUDA kernels handle vector similarity computations.

Warm Loading / VRAM Handoff — The encoder is flushed from GPU memory immediately after retrieval, clearing space for LLM inference and preventing Out-of-Memory (OOM) errors.
...
K-Means partitions the vector space into K Voronoi cells dynamically based on dataset size.

Coarse Search

Query is matched to the nearest centroid.

Fine Search

k-NN search is performed only within the relevant cluster.
